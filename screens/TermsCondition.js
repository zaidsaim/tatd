...
const TermsCondition = ({navigation, route}) => {
  ...
  const {item, priceamount} = route?.params;
  ...
  const updateCash = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://www.tatd.in/app-api/customer/ondemand/update_cash_option_booking_validation.php',
        {
      ...
      setUpdatecashdata(response.data);
    } catch (err) {
      setError(err.message);
  ...
  const createBooking = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://www.tatd.in/app-api/customer/ondemand/create-booking-by-booking_validation_id.php',
        {
      ...
      await AsyncStorage.setItem(
        'bookingidvalue',
        JSON.stringify(response.data?.booking_id),
      );
      if (response.data.status_code === 200) {
